...
Permission is granted to use this code, with or without modification, in any website or application provided that the following conditions are met:

   1. Credit is given to the original work with a link back to PrayTimes.org.
   2. Redistributions of the source code and its translations into other programming languages must retain this copyright notice.

This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY. PLEASE DO NOT REMOVE THIS COPYRIGHT BLOCK.
...
function PrayTime() {
	...
	setIshaaMinutes (minutes)		// minutes after maghrib
	...
	*/
	...
	this.calcMethod   	= 2;		// calculation method - (ISNA)
	this.asrJuristic  	= 0;		// Juristic method for Asr - (Shafii)
	this.zuhrMinutes 		= 0;		// minutes after mid-day for Zuhr - (0)
	this.maghribMinutes	= 6;		// minutes after sunset for maghrib - (6)
	this.adjustHighLats 	= 1;		// adjusting method for higher latitudes - (Middle of the night)
	...
	/*  this.methodParams[methodNum] = new Array(fa, ms, mv, is, iv);	
			...
			is : ishaa selector (0 = angle; 1 = minutes after maghrib)
			iv : ishaa parameter value (in angle or minutes)
	*/

	this.methodParams[this.Jafari]	= new Array(16, 0, 4, 0, 14);		
	this.methodParams[this.Karachi]	= new Array(18, 1, 0, 0, 18);		
	this.methodParams[this.ISNA]		= new Array(15, 1, 0, 1, 90);	// Final adjustment - fa=15, ms=1, mv=0, is=1, iv=90
...
// set the angle for calculating Ishaa
PrayTime.prototype.setIshaaAngle = function(angle) {
	this.setCustomParams(new Array(null, null, null, 0, angle));
}

// set the minutes after mid-day for calculating Zuhr
PrayTime.prototype.setZuhrMinutes = function(minutes) {
	this.zuhrMinutes = minutes;
}
...
// set the minutes after Maghrib for calculating Ishaa
PrayTime.prototype.setIshaaMinutes = function(minutes) {
...
// compute mid-day (Dhuhr, Zuhr, Zawal) time
...
PrayTime.prototype.computeTime = function(G, t) {
	...
	var V = 1/15* this.darccos((-this.dsin(G)- this.dsin(D)* this.dsin(this.lat)) / (this.dcos(D)* this.dcos(this.lat)));
	return Z+ (G>90 ? -V : V);
}
...
// ----------- NEW compute prayer times at given julian date ---------------------
PrayTime.prototype.computeTimes = function(times) {
	...
	var Zuhr   	= this.computeMidDay(t[2]);
	var Asr     = this.computeAsr(1+ this.asrJuristic, t[3]);
	var Maghrib = this.computeTime(0.833, t[4]);;
	var Ishaa   = this.computeTime(this.methodParams[this.calcMethod][4], t[5]);

	return new Array(Fajr, Sunrise, Zuhr, Asr, Maghrib, Ishaa);
}

// compute prayer times at given julian date
PrayTime.prototype.computeDayTimes = function() {
//	var times = new Array(5, 6, 12, 13, 18, 18, 18); 	//default times
	var times = new Array(5, 6, 12, 13, 18, 18); 		//default times
...
//alert("var nightTime is: "+nightTime)
...
//alert("var FajrDiff is: "+FajrDiff)

	// Adjust Ishaa
	var IshaaAngle = (this.methodParams[this.calcMethod][3] == 0) ? this.methodParams[this.calcMethod][4] : 18;
	var IshaaDiff = this.nightPortion(IshaaAngle)* nightTime;
	if (isNaN(times[6]) || this.timeDiff(times[4], times[6]) > IshaaDiff) 
